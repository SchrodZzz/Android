...
class LoaderDrawAnimationView @JvmOverloads constructor(
    ...
    private var circleRadius = dp(6.5f)

    private var circleScaleFactor = 1.3f
    private var paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = 0xFFe1e3e6.toInt()
    }
    private val betweenCircles = dp(24f)

    private val rectLongSide = dp(55f)
    private var rectShortSide = dp(14f)
    private val rectHypot = hypot(rectLongSide, rectShortSide)
    private val rectF = RectF()

    private var elementsMargin = dp(25f)
    private val topMargin = dp(2f)
    ...
    init {
        ...
        try {
            rectShortSide = dp(a.getFloat(R.styleable.LoaderDrawAnimationView_rectShortSide, 14f))
            circleRadius = dp(a.getFloat(R.styleable.LoaderDrawAnimationView_circleRadiuss, 6.5f))
            elementsMargin = dp(a.getFloat(R.styleable.LoaderDrawAnimationView_elementsMargin, 25f))
            animationDelay =
